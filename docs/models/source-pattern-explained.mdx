...
**Relationship:** `E36 Visual Item --P138 represents--> E22 Human-Made Object`

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#e8f4ea'}}}%%
flowchart LR
    E36_IMG["E36 Visual Item
    ...
    class E22 crm_E18_Physical_Thing
    class E36_IMG crm_E28_Conceptual_Object
```

The IIIF scan that appears on your screen is not the map. It _represents_ the map.

E36 Visual Item is a subclass of E73 Information Object --- it lives in the conceptual realm. The scan is a pattern of pixels that was mechanically derived from the physical artifact by a scanner or camera, but it is an entirely different kind of entity.
...
That word "responsibility" is significant. When a cartographer labels a plantation on a map, they are taking responsibility for that identification. When we later link that label to a specific E25 Plantation entity, the production event tells us _who made that claim_ and _when_.
...
- `E36 Visual Item --P138 represents--> E25 Human-Made Feature (Plantation)`
...
flowchart TB
    ...
    class E36 crm_E28_Conceptual_Object
    class E25,E26 crm_E18_Physical_Thing
...
The map's visual content (E36) represents the _plantation_ (E25). The plantation, being a physical thing in the real world, _has a location_ (E53). The map does not depict the location; it depicts the thing that is at the location. This is a subtle but crucial distinction:

- If E36 linked directly to E53, we would be saying "the map shows a piece of empty land."
- By routing through E25, we say "the map shows a plantation, and that plantation is located somewhere."
...
### Why E25 for plantations?

> **CRM Scope Note (E25 Human-Made Feature):**
> _"This class comprises physical features of any size that are purposely created by human activity. This class is a subclass of both E24 Physical Human-Made Thing and E26 Physical Feature."_

A plantation is a physical transformation of the landscape --- forests cleared, fields laid out, buildings constructed, canals dug. It is not just a concept or a legal designation; it is a persistent physical thing that was created by human activity and that you could walk through, touch, and photograph. E25 captures this physical reality as both a human creation and a landscape feature.
...
- `E41 Appellation (map label) --P1i identifies--> E25 Human-Made Feature`
...
P139 links "Geijersvlijt" (map label) to "Geyers-Vlijt" (almanac name) as alternative forms of each other. This does _not_ mean E25 and E74 are the same entity. It means: these two names, despite their different spellings and different referent types, ultimately trace back to the same real-world referent complex. The plantation (E25) and the organization (E74) are connected through ownership (P52, in the next layer), and their names are connected through P139.
...
- `E25 Human-Made Feature --P53 has former or current location--> E53 Place`
- `E25 Human-Made Feature --P52 has current owner--> E74 Group`
...
Plantations are, by nature, immobile. They were carved out of specific pieces of Surinamese land, and that land does not move. P53 connects the plantation (E25) to its location (E53), where E53 holds the actual geometry (a GeoSPARQL polygon derived from the 1930 QGIS map data).
...
E53 is purely spatial --- it is a piece of the Earth's surface, nothing more. It does not care who owned the land above it, or what was built on it, or what happened there. That information lives on E25 and E74. The separation is clean: **E53 = where**, **E25 = what**, **E74 = who**.

This is why the model routes location through E25 rather than attaching it to E36 or E22 directly. The map (E22) depicts the plantation (E25), and the plantation occupies a location (E53). Remove the map from the picture, and the plantation is still there, still occupying that same piece of land. Location is a property of the real-world thing, not of the document that depicted it.
...
P52 connects the physical plantation to the organization that owns or operates it. This is the relationship that makes E25 and E74 separate entities rather than one dual-typed entity (a key modeling decision, number 2 in our decisions list).

Why separate them? Because they change independently:

- A plantation can change owners. The land, the buildings, the canals --- the E25 --- remain the same. But the E74 connected through P52 changes.
- An organization can be dissolved when absorbed by another organization (`P99i was dissolved by` -> E68 Dissolution). The E74 entities merge, but the E25 might keep its physical boundaries.
- A plantation can be abandoned. The E25 still exists (you can still see its remains), but no E74 currently operates it.
...
> A **physical source** (E22), **produced** (E12) at a **time** (E52) by **someone** (E21/E74), **carries** (P128) **visual content** (E36) and **names** (E41). The content **represents** (P138) real-world **things** (E25, E26, E74, E21). The names **identify** (P1i) those things. And those things **have locations** (P53 to E53) and **owners** (P52 to E74). A **digital scan** (E36) **represents** (P138) the physical source, giving us access to everything it carries.
...
| P108 has produced                  | E12 Production      | E25 Human-Made Feature | Source was made by someone at some time          |
...
| ---------------------- | --------------------- | --------------------------------------- |
| Brown (#c78e66)        | E18 Physical Thing    | E22 Source, E25 Plantation, E26 Feature |
| Yellow (#fddc34)       | E28 Conceptual Object | E36 Visual Item                         |
