...
> **Model target:** `E24 Plantation` --- `P52 has current owner` --> `wd:{Q-ID}` (E74 Organization)
> **Coverage:** 1,062 / 1,597 (66%)

**Verdict: Works with caveat.** The Q-ID is the key that links the QGIS polygon (via E24) to the Wikidata organization (E74). The mapping is the cornerstone of the entire linking strategy:

```
E24 plantation/{slug} --P52 has current owner--> E74 wd:{Q-ID}
```
...
> **Model target:** `E24 Plantation` --- `skos:prefLabel` (display convenience)
> **Coverage:** 1,063 / 1,597 (67%)

**Verdict: Works.** This is the human-readable display label for the plantation. The model treats `skos:prefLabel` as a convenience alongside the formal E41 chain. Coverage is close to the Q-ID coverage (1,063 vs 1,062) --- almost exactly the rows that have been identified get a display label. When present, it is used as the `{name-slug}` in URI patterns.
...
> **Model target:** `E41 Appellation` --- `P190 has symbolic content` (text); `P1i identifies` E24; `P128i is carried by` E22 (1930 map source)
> **Coverage:** 1,032 / 1,597 (65%)

**Verdict: Works with caveat.** This is the name as it appears on the 1930 map --- the cartographic label. In CRM terms, the 1930 map (E22) _carries_ this appellation (E41), and the appellation _identifies_ the plantation (E24). The name provenance is clean:

```
E22 Map1930 --P128 carries--> E41 "Nieuw Java" --P1i identifies--> E24 Plantation
```
...
> **Sample:** `Nieuw Java`, `Destombesburg`, `Aan De Vier Kinderen`
> **Model target:** `E41 Appellation` --- `P190 has symbolic content`; `P1i identifies` E24; `P128i is carried by` E22 (1860--79 map source); linked to `label_1930` via `P139 has alternative form`
...
```
E22 Map1860 --P128 carries--> E41 "Destombesburg" --P1i identifies--> E24 Plantation
E22 Map1930 --P128 carries--> E41 "Destombesburg" --P1i identifies--> E24 Plantation
...
| `plantation_label` | E24 skos:prefLabel              | 67%      | Works                                       |
...
**Verdict: Works.** This is the standardized (modern-spelling) form of the plantation name. In the model, it creates an E41 Appellation instance identifying the E74 organization. Since this is a _standardized_ name, it is also used as the `skos:prefLabel` on E74.
...
> **Model target:** `E41 Appellation` --- `P190 has symbolic content`; `P72 has language` --> Sranantongo; `P1i identifies` E24 Plantation
...
In CRM terms, this is another E41 Appellation instance, but with a crucial difference: `P72 has language` points to Sranantongo (ISO 639-3: `srn`), not Dutch. It identifies the E24 Plantation (the physical place), not the E74 Organization (the colonial legal entity):

```
E22 Almanac --P128 carries--> E41 "..." (srn) --P1i identifies--> E24 Plantation
```
...
**Verdict: Works.** Straightforward numeric property. TvO notes it can change over time (plantations grew or shrank), which is exactly why it belongs on the observation rather than on the E24 or E74 directly. The 19% coverage reflects that size information was not consistently recorded across all edition years.
